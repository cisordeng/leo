...
func TestSet(t *testing.T) {
	safeMap = NewBeeMap()
	if ok := safeMap.Set("cisordeng", 1); !ok {
...
func TestCheck(t *testing.T) {
	if exists := safeMap.Check("cisordeng"); !exists {
		t.Error("expected", true, "got", false)
	}
}

func TestGet(t *testing.T) {
	if val := safeMap.Get("cisordeng"); val.(int) != 1 {
		t.Error("expected value", 1, "got", val)
	}
}

func TestDelete(t *testing.T) {
	safeMap.Delete("cisordeng")
	if exists := safeMap.Check("cisordeng"); exists {
...
func TestItems(t *testing.T) {
	safeMap := NewBeeMap()
	safeMap.Set("cisordeng", "hello")
	for k, v := range safeMap.Items() {
		key := k.(string)
		value := v.(string)
		if key != "cisordeng" {
			t.Error("expected the key should be cisordeng")
		}
		if value != "hello" {
